...
import type { AwsIamConnectionParams, PostgresNodeOptions } from '../helpers/interfaces';
import { configureQueryRunner, getConnectionParams } from '../helpers/utils';
...
export async function router(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	const connectionParams = getConnectionParams.call(this, 0) as AwsIamConnectionParams;
	const options = this.getNodeParameter('options', 0, {}) as PostgresNodeOptions;
	const node = this.getNode();
	options.nodeVersion = node.typeVersion;
	options.operation = operation;

	const { db, pgp } = await configurePostgres.call(this, connectionParams, options);
